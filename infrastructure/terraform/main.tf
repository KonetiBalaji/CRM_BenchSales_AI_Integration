# BenchCRM Production Infrastructure
terraform {
  ...
  backend "s3" {
    bucket = "benchcrm-terraform-state"
...
resource "aws_ecs_task_definition" "api" {
  ...
  container_definitions = jsonencode([
    {
      name  = "api"
      image = "${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.aws_region}.amazonaws.com/${aws_ecr_repository.api.name}:latest"
...
resource "aws_secretsmanager_secret" "openai_api_key" {
  ...
  description             = "OpenAI API Key for BenchCRM"
...
resource "aws_secretsmanager_secret" "stripe_secret_key" {
  ...
  description             = "Stripe Secret Key for BenchCRM"
  recovery_window_in_days = var.environment == "production" ? 30 : 0
